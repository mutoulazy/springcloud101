...
public class InvestServiceController implements InvestService {
  ...
  public Invest createOrder(long userId, long projectId, BigDecimal amount) throws Exception {
    ...
    if (amount.compareTo(investor.getAvailableBalance()) > 0) {
      throw new Exception("用户余额不足");
    }

    Project project = remoteProjectService.getProject(projectId);
    if (project == null) {
      throw new Exception("无效项目ID");
    }
    if (amount.compareTo(project.getRemainAmount()) > 0) {
      throw new Exception("超过项目可接收募捐数");
    }
    if (project.getStatus() != 1) {
      throw new Exception("项目不是募集中状不能投资");
    }
    ...
    if (remoteUserService.consumeMoney(userId, amount) == null) {
      throw new Exception("用户消费失败");
    }
    if (remoteProjectService.gotInvested(projectId, amount) == null) {
      throw new Exception("项目投资失败");
    }
